...
Pure filesystem validation against the component rubric.
...
def check_anti_patterns_section(content: str) -> CheckResult:
    """Check: Has patterns section heading (10 pts)."""
    ...
    return CheckResult("Patterns section", 10, 0, "No patterns heading found")
...
def check_routing_registration(component_type: str, file_path: Path, fm: dict | None) -> CheckResult:
    ...
    if component_type == "agent":
        ...
        return CheckResult("Registered in routing", 10, 0, f"'{skill_dir_name}' not in routing or INDEX")
...
def score_component(file_path: Path, do_check_secrets: bool = False) -> ComponentScore:
    ...
    fm = extract_frontmatter(content)
    score = ComponentScore(file_path=str(file_path.relative_to(REPO_ROOT)), component_type=component_type)
...
def score_to_dict(score: ComponentScore) -> dict:
    return {
        ...
        "grade": score.grade,
        "checks": [
            {"name": c.name, "status": c.status, "earned": c.earned_points, "max": c.max_points, "detail": c.detail}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("files", nargs="*", help="Component file(s) to score")
    parser.add_argument("--all-agents", action="store_true", help="Score all agents in agents/")
    parser.add_argument("--all-skills", action="store_true", help="Score all skills in skills/*/SKILL.md")
    parser.add_argument("--check-secrets", action="store_true", help="Enable secret detection")
...
def main() -> int:
    ...
    if not unique_targets:
        print("Error: No files to score.", file=sys.stderr)
        return 2
